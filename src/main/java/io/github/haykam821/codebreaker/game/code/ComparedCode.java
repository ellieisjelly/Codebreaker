...
public class ComparedCode extends Code {
	...
	public void build(LevelAccessor level, BlockPos originPos, CodebreakerMapConfig mapConfig) {
		super.build(level, originPos, mapConfig);
		
		BlockPos.MutableBlockPos pos = originPos.mutable().move(0, -this.getLength(), 1);
		for (int index = 0; index < this.getLength(); index++) {
			if (index >= this.results.size()) {
				level.setBlock(pos, CodeResult.EMPTY, 3);
			} else {
				CodeResult result = this.results.get(index);
				level.setBlock(pos, result == null ? CodeResult.EMPTY : result.getState(), 3);
			}
